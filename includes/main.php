<?php

/*
	This is the main include file.
	It is only used in index.php and keeps it much cleaner.
*/

require_once "includes/config.php";
require_once "includes/connect.php";
require_once "includes/helpers.php";
require_once "includes/models/place.model.php";
require_once "includes/models/user.model.php";
require_once "includes/models/otheruser.model.php";
require_once "includes/models/friend.model.php";
require_once "includes/models/follower.model.php";
require_once "includes/models/placedetail.model.php";
require_once "includes/controllers/home.controller.php";
require_once "includes/controllers/place.controller.php";
require_once "includes/controllers/user.controller.php";
require_once "includes/controllers/otheruser.controller.php";
require_once "includes/controllers/friend.controller.php";
require_once "includes/controllers/placedetail.controller.php";
require_once "includes/controllers/login.controller.php";
require_once "includes/controllers/logout.controller.php";

// This will allow the browser to cache the pages of the store.

//header('Cache-Control: max-age=3600, public');
//header('Pragma: cache');
//header("Last-Modified: ".gmdate("D, d M Y H:i:s",time())." GMT");
//header("Expires: ".gmdate("D, d M Y H:i:s",time()+0)." GMT");

?>